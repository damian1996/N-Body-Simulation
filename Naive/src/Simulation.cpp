...
=======
Simulation::Simulation(Render *r, Step *comp, unsigned N)
    : r(r), comp(comp), N(N) {
  rg = new RandomGenerators();
  positions.reserve(3 * N);
  for (unsigned i = 0; i < N; i++) {
    for (int j = 0; j < 3; j++) {
      positions.push_back(rg->getRandomdouble(-parsecsWidth, parsecsWidth));
...
void Simulation::normalizePositions() {
  for (unsigned i = 0; i < 3 * N; i++) {
    posToDraw[i] = (positions[i] / parsecsWidth); // /200.0;
  }
}
...
void Simulation::makeSimulation() {
  ...
  while (true) {
    ...
    comp->compute(positions,
                  (curr_time - last_time) * 10); // 100*(curr_time-last_time));
    last_time = curr_time;
    /*
    normalizePositions();
...
// src/Simulation.cpp:14:12: warning: deleting object of abstract class type
// ‘Step’ which has non-virtual
